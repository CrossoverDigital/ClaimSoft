...
using CD.ClaimSoft.Database.Model.Authorization.Account;
...
namespace CD.ClaimSoft.UI.Areas.Account.Controllers
{
    [Authorize]
    public class AccountController : BaseController
    {
        ...
        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager, IAuthenticationManager authManager, IClaimSoftContext db, IRedisCache cache, ILogService<ApplicationUserManager> logService)
        {
        ...
        public async Task<ActionResult> Login(Login model, string returnUrl)
        {
            ...
            try
            {
                ...
                switch (result)
                {
                    case SignInStatus.Success:
                        var identity = SignInManager.AuthenticationManager.AuthenticationResponseGrant.Identity;

                        // If successful, then we need to set a few session items
                        var userAgencies = new UserProfileHelper().UserAgencies(identity.GetUserId());
                        Session[CacheConstants.UserAgenciesKey] = userAgencies;

                        if (userAgencies.Count() == 1)
                        {
                            Session[CacheConstants.CurrentUserAgencyKey] = userAgencies[0];
                        }
            ...
            {
                // _logService.Warn("Invalid login attempt.");
        ...
        public async Task<ActionResult> Register(Register model)
        {
            if (ModelState.IsValid)
            {
                var context = new ClaimSoftContext();
                //you can try to get accountId field from session
                var accountId = 1;
                var agency = context.AgencyUsers.Find(accountId);
                if (agency != null)
                {
